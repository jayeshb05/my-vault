...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const savedFiles = files.filter(f => f && f.size > 0).length;
    const savedNotes = textContent.trim() ? 1 : 0;
    const totalSaved = savedFiles + savedNotes;
    return NextResponse.redirect(new URL(`/share?done=1&files_saved=${savedFiles}&total=${totalSaved}`, req.url));
  } catch {
