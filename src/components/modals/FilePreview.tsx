...
export default function FilePreview() {
  ...
  const handleClose = () => setShowFilePreview(false);
  ...
  const handleCopy = async () => {
    setCopying(true);
    try {
      await copyVaultItem(selectedItem);
    } catch {
      /* ignore */
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-3xl h-[92dvh] sm:h-[88dvh] bg-[var(--bg-card)] rounded-t-2xl sm:rounded-2xl border border-[var(--border)] shadow-2xl flex flex-col animate-slide-up">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] shrink-0">
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            <button
              onClick={handleCopy}
              className="p-2 rounded-lg hover:bg-[var(--bg-hover)] active:bg-[var(--bg-hover)] text-[var(--text-muted)]"
              title="Copy"
            >
              {copying
                ? <span className="text-[10px] font-medium text-[var(--accent)] px-1">✓</span>
            ...
            <button
              ...
              className="p-2 rounded-lg hover:bg-red-500/10 active:bg-red-500/10 text-red-500"
              title="Delete"
            >
